...
Per-source external IDs (HubSpot Contact ID, Pipedrive Person ID, Go High Level Contact ID, Chapter Pro Member ID, etc.) live in the generic `member_external_ids` table — one row per (member, source) — NOT as dedicated columns on the `members` table. This keeps the schema connector-agnostic: adding a new CRM is a connector implementation plus per-chapter config change, never a schema migration. See `docs/adr/ADR-003-multi-crm-external-ids.md` for full rationale and alternatives considered.

  

Exception: `eo_global_member_id` stays a top-level column on `members` because it has unique canonical meaning across all EO chapters (v1.1 §2.4 — cross-chapter deduplication key), not just one source.
